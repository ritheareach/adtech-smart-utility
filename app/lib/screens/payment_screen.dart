...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  Bill? get _selectedBill =>
      _selectedBillIndex != null ? _bills[_selectedBillIndex!] : null;
  ...
  Future<void> _loadBills() async {
    ...
    try {
      final bills = await ApiService().getCurrentBills();
      if (!mounted) return;
  ...
  void _selectMethod(String method) {
    ...
    if (method == 'card') {
      Navigator.push(context, MaterialPageRoute(
        builder: (_) => CardPaymentScreen(bill: _selectedBill!),
      ));
    } else {
      Navigator.push(context, MaterialPageRoute(
        builder: (_) => KhqrScreen(bill: _selectedBill!, method: method),
      ));
    }
  }
                          ...
                          Row(
                            children: [
                              ...
                              Text('Due on ${bill.dueDate}',
                                  style: const TextStyle(fontSize: 11, color: AppColors.orange, fontWeight: FontWeight.w500)),
                            ],
                          ),
                        ],
                      ),
